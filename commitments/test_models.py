...
class CommitmentProfileTestCase(TestCase):
    ...
    def test_user_with_profile(self):
        """make sure that the accountableuser gets automatically generated"""
...
class CommitmentSnapshotTestCase(TestCase):
    ...
    def test_create_today_snapshot(self):
        snapshot = self.user.accountableuser.get_snapshot()
        assert snapshot
        duplicate_snapshot = self.user.accountableuser.get_snapshot()
    ...
    def test_create_future_snapshot(self):
        snapshot = self.user.accountableuser.get_snapshot(date=datetime.date.today()+datetime.timedelta(days=1))
        assert snapshot

    def test_set_commitment_accomplished(self):
        all_commitments = self.user.accountableuser.get_active_commitments()
        snapshot = self.user.accountableuser.get_snapshot()
